...
Target "Build" (fun _ ->
  DotNetCli.Build (fun p-> { p with Project = @".\src\Serilog.Sinks.YouTrack.sln"
      ...
      (fun p -> 
            {
              p with             
                  Configuration = buildMode
                  Project = project
                  AdditionalArgs = ["--filter Category!=Integration"]
            }) 
  )       
)
...
"Clean"
  ...
  ==> "Build"
  ==> "Test"
  =?> ("IntegrationTest", match environVarOrNone "youtrack-connection" with | Some(x) -> true | _ -> false)
  ==> "Package"
  ==> "Default"
